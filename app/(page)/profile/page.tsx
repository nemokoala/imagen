...
import { useGetUserImagesQuery } from "@/queries/image/queries";
...
export default function ProfilePage() {
  ...
  const { data: userImagesData, isLoading: imagesLoading } =
    useGetUserImagesQuery(userInfo?.id || 0, 1, 20);
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="mb-4">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-6">
            총 {userImagesData?.totalCount || 0}개의 이미지
          </p>
        </div>
        ...
        ) : userImagesData && userImagesData.images.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {userImagesData.images.map((image) => (
              <ImageCard
                key={image.id}
                image={image}
